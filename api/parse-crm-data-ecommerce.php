...
error_log("=== E-COMMERCE API CALLED ===");
...
function parseProcessTimeData($html) {
    ...
    if (preg_match('/<table[^>]*class="[^"]*shop-table[^"]*"[^>]*>([\s\S]*?)<\/table>/i', $html, $tableMatch)) {
        ...
        if (preg_match_all('/<tr([^>]*)>([\s\S]*?)<\/tr>/i', $tableHtml, $trMatches, PREG_SET_ORDER)) {
            foreach ($trMatches as $trMatch) {
                ...
                if ($hasOnclick && !$hasDataGroup) {
                    ...
                    // Основной, более строгий паттерн (как было раньше)
        ...
        // Если строгий паттерн ничего не нашёл (как в ДИАР/Трансляции) —
        // делаем более мягкий разбор: ищем "N (M процес…)" в тексте таблицы
...
try {
    ...
    error_log("Login successful for E-COMMERCE, User: " . $config['login']);
    ...
    for ($attempt = 1; $attempt <= $maxAttempts; $attempt++) {
        ...
        // Нулевые данные — это тоже валидный результат (нет заказов = нет заказов).
        if ($filteredData && $totalData) {
            error_log("Data fetched successfully on attempt $attempt");
            break;
        } else {
