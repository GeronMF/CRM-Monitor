...
function parseProcessTimeData($html) {
    ...
    if (preg_match('/<table[^>]*class="[^"]*shop-table[^"]*"[^>]*>([\s\S]*?)<\/table>/i', $html, $tableMatch)) {
        ...
        // Если ничего не нашли строгим паттерном (частый случай под Захарченко) —
        // пробуем более мягкий поиск по тексту всей таблицы.
...
try {
    ...
    error_log("Login successful for DIAR, User: " . $config['login']);
